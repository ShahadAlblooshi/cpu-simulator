// script.js - Full corrected Mano-style CPU simulator with indirect addressing
...
function microStep() {
  ...
  if (opcodeTop!==0x7) { // memory ref
    ...
    // Only read memory if not STA (store) instruction
    if (SC===3) {
...
document.getElementById('btnInst').addEventListener('click', ()=>{ if(halted){ pushTrace('Halted'); return;} while(SC!==0 || !halted){ microStep(); if(SC===0) break;} updateUI(); });
document.getElementById('btnRun').addEventListener('click',()=>{ if(runTimer) return; const speed=Math.max(10, parseInt(document.getElementById('speed').value,10)||400); runTimer=setInterval(()=>{ if(halted){ clearInterval(runTimer); runTimer=null; pushTrace('Stopped'); return;} microStep(); updateUI(); },speed); });
...
document.getElementById('btnReset').addEventListener('click',()=>{ MEM=new Array(MEM_SIZE).fill('0000'); PC=AR=IR=AC=DR=E=0; SC=0; halted=false; profiler={cycles:0,instr:0,reads:0,writes:0}; updateUI(['PC','AR','IR','AC','DR']); pushTrace('Simulator reset'); });
